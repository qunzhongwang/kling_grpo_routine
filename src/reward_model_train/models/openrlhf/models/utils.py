...
def masked_mean(tensor: torch.Tensor, mask: Optional[torch.Tensor], dim: int = None) -> torch.Tensor:
    ...
    return (tensor * mask).sum(axis=dim) / 200. # (mask.sum(axis=dim)+1e-4)
...
def packed_sequence_to_position_tensor(packed_seq_lens, device):
    """
    Converts packed_seq_lens to a tensor of token positions.

    Args:
        packed_seq_lens: A list of integers representing token length for each sequence.

    Returns:
        A tensor of shape (1, ntoken) containing the sequences of positions.
    """
    output_list = []
    for seq_len in packed_seq_lens:
        output_list.extend(list(range(seq_len)))
    return torch.tensor(output_list, device=device).unsqueeze(0)
